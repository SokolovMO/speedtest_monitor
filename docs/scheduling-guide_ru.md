...
1. **В config.yaml** - указываете желаемый интервал:
   ```yaml
   telegram:
     check_interval: 3600  # секунды
   ```

2. **В systemd timer ИЛИ cron** - настраиваете соответствующее расписание

⚠️ **Важно:** Значения должны совпадать!

---

## 📊 Таблица соответствий

| Интервал | config.yaml | systemd timer | cron |
|----------|-------------|---------------|------|
| **15 минут** | `check_interval: 900` | `OnCalendar=*:0/15` | `*/15 * * * *` |
| **30 минут** | `check_interval: 1800` | `OnCalendar=*:0/30` | `*/30 * * * *` |
| **1 час** | `check_interval: 3600` | `OnCalendar=hourly` | `0 * * * *` |
| **2 часа** | `check_interval: 7200` | `OnCalendar=0/2:00` | `0 */2 * * *` |
| **6 часов** | `check_interval: 21600` | `OnCalendar=0/6:00` | `0 */6 * * *` |
| **12 часов** | `check_interval: 43200` | `OnCalendar=0/12:00` | `0 */12 * * *` |
