# 📅 Шпаргалка по настройке расписания

## 🎯 Быстрый старт

### Где настраивается частота?

Настройка зависит от режима работы:

1.  **Single / Node Mode**:
    -   Частота контролируется **Systemd Timer** (внешний таймер).
    -   Настройки в `config.yaml` используются только для справки или порогов.

2.  **Master Mode**:
    -   Частота контролируется **Внутренним планировщиком** (`config.yaml`).
    -   Systemd запускает сервис как постоянный процесс (`speedtest-master.service`).

---

## 📊 Расписание по режимам

### 1. Single & Node Mode (Внешний таймер)

В этих режимах Python-скрипт запускается один раз, выполняет тест и завершается. Systemd Timer запускает его периодически.

| Интервал | systemd timer (`speedtest-monitor.timer`) |
|----------|-------------------------------------------|
| **15 мин** | `OnCalendar=*:0/15` |
| **1 час** | `OnCalendar=hourly` |
| **4 часа** | `OnCalendar=0/4:00` |

**Настройка:**
Редактировать `/etc/systemd/system/speedtest-monitor.timer`:
```ini
[Timer]
OnCalendar=hourly
```

### 2. Master Mode (Внутренний планировщик)

В режиме Master сервис работает постоянно (`Type=simple`). Он собирает отчеты от узлов и отправляет сводные данные на основе `config.yaml`.

**Настройка:**
Редактировать `config.yaml`:
```yaml
master:
  schedule:
    interval_minutes: 60      # Отправлять отчет каждые 60 минут
    send_immediately: false   # Ждать интервала (не спамить)
```

| Параметр | Описание |
|----------|----------|
| `interval_minutes` | Как часто отправлять агрегированный отчет в Telegram. |
| `send_immediately` | Если `true`, отправляет отчет *каждый раз* при обновлении данных узла (может спамить). Если `false`, агрегирует и отправляет раз в интервал. |

### 3. Локальная нода на Master (Внешний таймер)

Если вы установили локальную ноду на мастере, у нее есть свой собственный таймер.

**Настройка:**
Редактировать `/etc/systemd/system/speedtest-master-node.timer`:
```ini
[Timer]
OnBootSec=5min
OnUnitActiveSec=60min
```

---

## 🔧 Конфигурация Systemd

### Node / Single Mode
Использует **Service + Timer**.

**speedtest-monitor.service**:
```ini
[Service]
Type=oneshot
ExecStart=/opt/speedtest-monitor/.venv/bin/python -m speedtest_monitor.main
```

**speedtest-monitor.timer**:
```ini
[Timer]
OnCalendar=hourly
```

### Master Mode
Использует **Только Service** (Демон).

**speedtest-master.service**:
```ini
[Service]
Type=simple
Restart=always
ExecStart=/opt/speedtest-monitor/.venv/bin/python -m speedtest_monitor.main
```

### Локальная нода на Master
Использует **Service + Timer** (Отдельно от Master).

**speedtest-master-node.service**:
```ini
[Service]
Type=simple
Environment="CONFIG_PATH=/opt/speedtest-monitor/config-master-node.yaml"
ExecStart=/opt/speedtest-monitor/.venv/bin/python -m speedtest_monitor.main
```

**speedtest-master-node.timer**:
```ini
[Timer]
OnUnitActiveSec=60min
```

---

## 🔧 Systemd Timer (Legacy / Single Mode)

### Как работает

```
┌─────────────┐       ┌──────────────┐       ┌─────────────┐
│ Timer файл  │ ----> │ Service файл │ ----> │ Python тест │
│ (КОГДА)     │       │ (ЧТО)        │       │             │
└─────────────┘       └──────────────┘       └─────────────┘
```

### Установка

```bash
# 1. Скопировать файлы
sudo cp systemd/*.service /etc/systemd/system/
sudo cp systemd/*.timer /etc/systemd/system/

# 2. Настроить частоту (если нужно изменить)
sudo nano /etc/systemd/system/speedtest-monitor.timer

# 3. Включить и запустить
sudo systemctl daemon-reload
sudo systemctl enable speedtest-monitor.timer
sudo systemctl start speedtest-monitor.timer
```

### Проверка

```bash
# Статус таймера
systemctl status speedtest-monitor.timer

# Когда следующий запуск?
systemctl list-timers speedtest-monitor.timer

# Логи последнего выполнения
journalctl -u speedtest-monitor.service -n 50

# Логи в реальном времени
journalctl -u speedtest-monitor.service -f
```

### Изменение частоты

Файл таймера (`/etc/systemd/system/speedtest-monitor.timer`) содержит закомментированные примеры. Отредактируйте его:

```bash
sudo nano /etc/systemd/system/speedtest-monitor.timer
```

**Вариант 1: Интервальный запуск (рекомендуется)**

Запуск каждые X минут после завершения предыдущего:

```ini
[Timer]
OnBootSec=1min                # Старт через 1 мин после загрузки
OnUnitActiveSec=10min        # Запуск каждые 10 минут после завершения
AccuracySec=1min
```

**Вариант 2: По расписанию**

Запуск в конкретное время:

```ini
[Timer]
# Каждый час в :00
OnCalendar=hourly

# Каждые 30 минут (:00 и :30)
OnCalendar=*:0/30

# Каждые 10 минут
OnCalendar=*:0/10

# Ежедневно в полночь
OnCalendar=*-*-* 00:00:00

# Каждый понедельник в 9:00
OnCalendar=Mon *-*-* 09:00:00
```

**После изменений:**

```bash
sudo systemctl daemon-reload
sudo systemctl restart speedtest-monitor.timer
systemctl list-timers speedtest-monitor.timer
```

**Проверка синтаксиса расписания:**

```bash
systemd-analyze calendar "hourly"
systemd-analyze calendar "*:0/10"
```

**Документация:**
- `man systemd.timer`
- https://www.freedesktop.org/software/systemd/man/systemd.timer.html

После изменений:
```bash
sudo systemctl daemon-reload
sudo systemctl restart speedtest-monitor.timer
```

---

## ⏰ Cron (Альтернатива)

### Установка

```bash
# Открыть редактор crontab
crontab -e

# Вставить строку (выбрать нужный интервал)
```

### Примеры для разных интервалов

```bash
# Каждые 15 минут
*/15 * * * * cd /opt/speedtest-monitor && /opt/speedtest-monitor/.venv/bin/python -m speedtest_monitor.main >> /opt/speedtest-monitor/cron.log 2>&1

# Каждые 30 минут
*/30 * * * * cd /opt/speedtest-monitor && /opt/speedtest-monitor/.venv/bin/python -m speedtest_monitor.main >> /opt/speedtest-monitor/cron.log 2>&1

# Каждый час (в 0 минут)
0 * * * * cd /opt/speedtest-monitor && /opt/speedtest-monitor/.venv/bin/python -m speedtest_monitor.main >> /opt/speedtest-monitor/cron.log 2>&1

# Каждые 2 часа
0 */2 * * * cd /opt/speedtest-monitor && /opt/speedtest-monitor/.venv/bin/python -m speedtest_monitor.main >> /opt/speedtest-monitor/cron.log 2>&1

# Каждые 6 часов (00:00, 06:00, 12:00, 18:00)
0 0,6,12,18 * * * cd /opt/speedtest-monitor && /opt/speedtest-monitor/.venv/bin/python -m speedtest_monitor.main >> /opt/speedtest-monitor/cron.log 2>&1

# В конкретное время каждый день (09:00)
0 9 * * * cd /opt/speedtest-monitor && /opt/speedtest-monitor/.venv/bin/python -m speedtest_monitor.main >> /opt/speedtest-monitor/cron.log 2>&1
```

### Проверка cron

```bash
# Посмотреть текущие задания
crontab -l

# Посмотреть логи
tail -f /opt/speedtest-monitor/cron.log

# Проверить, работает ли cron
sudo systemctl status cron  # Debian/Ubuntu
sudo systemctl status crond  # CentOS/RHEL
```

### Формат cron в деталях

```
┌───────────── минута (0 - 59)
│ ┌─────────── час (0 - 23)
│ │ ┌───────── день месяца (1 - 31)
│ │ │ ┌─────── месяц (1 - 12)
│ │ │ │ ┌───── день недели (0 - 7, 0 и 7 = воскресенье)
│ │ │ │ │
* * * * * команда
```

**Специальные символы:**

| Символ | Значение | Пример |
|--------|----------|--------|
| `*` | Каждое значение | `* * * * *` = каждую минуту |
| `*/N` | Каждые N единиц | `*/15 * * * *` = каждые 15 минут |
| `N,M` | Конкретные значения | `0 9,12,18 * * *` = в 9:00, 12:00, 18:00 |
| `N-M` | Диапазон | `0 9-17 * * *` = каждый час с 9:00 до 17:00 |

---

## 🆚 Сравнение: что выбрать?

### Systemd Timer ✅

**Преимущества:**
- ✅ Не запустит дубликаты
- ✅ Встроенное логирование
- ✅ Управление зависимостями
- ✅ Автозапуск после перезагрузки
- ✅ Запуск пропущенных задач (Persistent=true)
- ✅ Легко мониторить (`systemctl status`)

**Недостатки:**
- ⚠️ Сложнее настроить (2 файла)
- ⚠️ Только на системах с systemd

**Рекомендуется для:**
- Серверов production
- Критичных систем
- Долгосрочной эксплуатации

### Cron ⏰

**Преимущества:**
- ✅ Проще настроить (1 строка)
- ✅ Универсален (работает везде)
- ✅ Знаком большинству админов

**Недостатки:**
- ❌ Может запустить несколько копий одновременно
- ❌ Нужно вручную настраивать логирование
- ❌ Переменные окружения могут не работать
- ❌ Нет автоматического управления зависимостями

**Рекомендуется для:**
- Быстрого прототипирования
- Простых задач
- Систем без systemd

---

## 🔍 Устранение неполадок

### Timer не запускается

```bash
# Проверить синтаксис
sudo systemd-analyze verify /etc/systemd/system/speedtest-monitor.timer

# Посмотреть ошибки
journalctl -xe

# Проверить, включен ли
systemctl is-enabled speedtest-monitor.timer

# Включить, если выключен
sudo systemctl enable speedtest-monitor.timer
```

### Cron не работает

```bash
# Проверить синтаксис crontab
crontab -l

# Проверить, запущен ли cron
sudo systemctl status cron

# Проверить логи системы
grep CRON /var/log/syslog  # Debian/Ubuntu
grep CRON /var/log/cron    # CentOS/RHEL

# Проверить права на скрипт
ls -la /opt/speedtest-monitor/.venv/bin/python

# Проверить путь к Python
which python
```

### Тест не выполняется

```bash
# Запустить вручную для проверки
cd /opt/speedtest-monitor
source .venv/bin/activate
python -m speedtest_monitor.main

# Проверить права доступа
ls -la config.yaml .env

# Проверить логи приложения
tail -f speedtest.log

# Для systemd:
journalctl -u speedtest-monitor.service -n 100 --no-pager
```

---

## 💡 Полезные команды

### Systemd

```bash
# Быстрая диагностика
systemctl status speedtest-monitor.timer
systemctl status speedtest-monitor.service
systemctl list-timers

# Запустить немедленно (для теста)
sudo systemctl start speedtest-monitor.service

# Посмотреть логи за сегодня
journalctl -u speedtest-monitor.service --since today

# Посмотреть только ошибки
journalctl -u speedtest-monitor.service -p err

# Очистить старые логи journal
sudo journalctl --vacuum-time=7d
```

### Cron

```bash
# Проверить синтаксис перед сохранением
crontab -l | crontab -

# Удалить все задания
crontab -r

# Редактировать с резервной копией
crontab -l > crontab.backup
crontab -e

# Восстановить из резервной копии
crontab crontab.backup

# Посмотреть задания другого пользователя (root)
sudo crontab -u username -l
```

---

## 📚 См. также

- [Полное руководство по конфигурации](configuration_ru.md)
- [Установка](installation_ru.md)
- [Развёртывание](deployment_ru.md)
- [Устранение неполадок](troubleshooting.md)
