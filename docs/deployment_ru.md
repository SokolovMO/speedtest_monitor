...
Самый простой способ развертывания — использовать скрипт `install.sh`, который автоматически создает и настраивает systemd сервисы.
...
### Справочник сервисов Systemd

Установщик создает следующие юниты в зависимости от выбранного режима:

| Режим | Файл юнита | Описание |
|-------|------------|----------|
| **Master** | `speedtest-master.service` | Основной сервис агрегатора (работает постоянно). |
| **Node** | `speedtest-monitor.timer` | Периодически запускает выполнение теста скорости. |
| **Node** | `speedtest-monitor.service` | Юнит выполнения теста скорости. |
| **Local Node** | `speedtest-master-node.timer` | Запускает локальный тест скорости на Master. |
| **Local Node** | `speedtest-master-node.service` | Юнит выполнения локального теста. |

---

## 🌐 Развертывание на нескольких серверах (Архитектура Master/Node)
...
**Ключевые моменты:**
- ✅ **Master**: Центральная точка. Требует открытого порта (по умолчанию 8080).
- ✅ **Nodes**: Запускают тесты и отправляют данные на Master. Входящие порты не требуются.
- ✅ **Telegram**: Только Master общается с Telegram API.

### Пошаговое руководство

**1. Развертывание Master**

```bash
# На сервере Master
./install.sh install master
# Настройте API Token, Порт и т.д.
```

**2. Развертывание Nodes**

```bash
# На серверах Node
./install.sh install node
# Настройте Node ID, URL Мастера, API Token
```

**3. Проверка связи**

Проверьте логи Master, чтобы увидеть входящие отчеты:
