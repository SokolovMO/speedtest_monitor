...
### 5. Локальная нода на Master (Опционально)

Если вы хотите, чтобы Master-сервер также выполнял тесты скорости и отправлял отчеты самому себе, вам нужен отдельный файл конфигурации (например, `config-master-node.yaml`).

Этот файл идентичен стандартной конфигурации Node, но указывает на `localhost`.

**Файл: `config-master-node.yaml`**

```yaml
mode: node

node:
  node_id: "master_node"  # Должен совпадать с записью в nodes_meta на Master
  description: "Master Server Local Node"
  
  # Подключение к localhost
  master_url: "http://127.0.0.1:8080/api/v1/report"
  
  # Тот же токен, что и в config.yaml
  api_token: "ВАШ_СГЕНЕРИРОВАННЫЙ_ТОКЕН"

# Стандартные настройки speedtest...
speedtest:
  timeout: 60
  retry_count: 3
  # ...
